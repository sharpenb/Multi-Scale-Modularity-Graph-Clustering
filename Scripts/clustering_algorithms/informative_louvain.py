...
def informative_louvain(G, resolution=1, eps=0.):
...
def maximize(G, resolution, eps):
    ...
    # total weight
    wtot = float(sum(list(node_weight.values())))
    # clusters
    ...
    e_1 = {u: {v: G[u][v]['weight'] / wtot * np.log(G[u][v]['weight'] / wtot) - resolution * G[u][v]['weight'] / wtot * np.log(node_weight[u] * node_weight[v] / wtot**2) for v in G.neighbors(u) if v != u} for u in G.nodes()}
    increase = True
    while increase:
        increase = False
        for u in G.nodes():
            ...
            if delta[l] - delta[k] > eps:
                ...
                for v in G.neighbors(u):
                    if v != u:
                        e_1[v][k] -= G[u][v]['weight'] / wtot * np.log(G[u][v]['weight'] / wtot) - resolution * G[u][v]['weight'] / wtot * np.log(node_weight[u] * node_weight[v] / wtot**2)
                        if e_1[v][k] == 0:
                            e_1[v].pop(k)
                        if l not in e_1[v].keys():
                            e_1[v][l] = 0
                        e_1[v][l] += G[u][v]['weight'] / wtot * np.log(G[u][v]['weight'] / wtot) - resolution * G[u][v]['weight'] / wtot * np.log(node_weight[u] * node_weight[v] / wtot**2)
    return cluster
